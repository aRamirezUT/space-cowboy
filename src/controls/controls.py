...
class Controls:
    ...
    def get_keyboard_inputs(inputs) -> Tuple[float, float]:
        """Return input states where 0 = down/relax, 1 = up/flex.

        Player 1 uses W for up. Player 2 uses UP arrow for up.
        Returns:
            1.0 when up key is held, 0.0 otherwise (for down/default movement).
        """
        player1_up = 0.0
        player2_up = 0.0
