...
class Pong():
    ...
    def update(self, dt: float):
        ...
        lrect = self._player_front_hitbox(self.left, facing_right=True)
        rrect = self._player_front_hitbox(self.right, facing_right=False)

        # Only reflect when approaching the player's front face and overlapping its front hitbox
        if ball_rect.colliderect(lrect) and self.ball.vx < 0:
            self._reflect_from_player(self.left, facing_right=True, hitbox=lrect)
        elif ball_rect.colliderect(rrect) and self.ball.vx > 0:
            self._reflect_from_player(self.right, facing_right=False, hitbox=rrect)
    ...
    def _reflect_from_player(self, player: Player, *, facing_right: bool, hitbox):
    ...
    def _player_front_hitbox(self, player: Player, *, facing_right: bool):
