...
class QuickdrawGame:
    def __init__(self, *, controls:Controls, screen: Optional[pygame.Surface] = None, own_display: bool | None = None):
        ...
        self.left = Player(MARGIN_X, base_y, PLAYER_W, PLAYER_H, HEIGHT, image_path=self.left_holstered)
        self.right = Player(WIDTH - MARGIN_X - PLAYER_W, base_y, PLAYER_W, PLAYER_H, HEIGHT, image_path=self.right_holstered)
    ...
    def _draw_player(self, player: Player, *, facing_right: bool, now_ms: int, is_winner: bool, is_kill_target: bool = False):
        ...
        if is_kill_target and self.kill_start_ms is not None:
            ...
            try:
                player._ensure_images()  # type: ignore[attr-defined]
                base = player._img_right if facing_right else player._img_left  # type: ignore[attr-defined]
            except Exception:
                base = None
            if base is not None:
                img = base.copy()
        ...
        if is_winner and self.win_anim_start_ms is not None:
            ...
            try:
                ...
                player._ensure_images()  # type: ignore[attr-defined]
                img = player._img_right if facing_right else player._img_left  # type: ignore[attr-defined]
            except Exception:
                img = None
            if img is not None:
        ...
        player.draw(self.scene, facing_right=facing_right, fg_color=FG_COLOR, accent=ACCENT)
